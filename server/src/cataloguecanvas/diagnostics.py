...
def build_report() -> str:
    ...
    out.append(f"- Allow external requests: {settings.allow_external_requests}")
    out.append(f"- Trusted proxies: {', '.join(sorted(settings.trusted_proxies)) or '(none)'}")
    out.append(f"- Activity log enabled: {settings.audit_log_enabled}")
    out.append(f"- Activity log: `{settings.audit_log_path}` (exists: {settings.audit_log_path.is_file()})")
